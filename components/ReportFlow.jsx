  ...
  if (done) {
    return (
      <div style={{
        ...
        <h2 style={{ fontSize: "22px", fontWeight: 700 }}>¡Reporte enviado!</h2>
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)", overflow: "auto" }}>
      ...
      <div style={{ padding: "24px 20px", flex: 1, overflowY: "auto" }}>
        ...
        {step === 0 && (
          <>
            <h2 style={{ fontSize: "20px", fontWeight: 700, margin: "0 0 4px", letterSpacing: "-0.3px" }}>¿Dónde está el arroyo?</h2>
            <p style={{ color: "var(--text-dim)", fontSize: "13px", margin: "0 0 20px" }}>Selecciona la zona afectada</p>
        ...
        {step === 1 && (
          <>
            <h2 style={{ fontSize: "20px", fontWeight: 700, margin: "0 0 4px", letterSpacing: "-0.3px" }}>¿Qué tan grave está?</h2>
            <p style={{ color: "var(--text-dim)", fontSize: "13px", margin: "0 0 24px" }}>Selecciona el nivel de riesgo</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {Object.entries(SEVERITY).map(([key, cfg], i) => (
                <button key={key} onClick={() => { setSeverity(key); setStep(2); }} style={{
                  ...
                  <div>
                    <div style={{ color: cfg.color, fontSize: "18px", fontWeight: 700 }}>{cfg.label}</div>
                    <div style={{ color: "var(--text-secondary)", fontSize: "13px", marginTop: 4 }}>{cfg.hint}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        {step === 2 && (
          <>
            <h2 style={{ fontSize: "20px", fontWeight: 700, margin: "0 0 4px", letterSpacing: "-0.3px" }}>¿Algo más que reportar?</h2>
            <p style={{ color: "var(--text-dim)", fontSize: "13px", margin: "0 0 20px" }}>Opcional — ayuda a otros a entender la situación</p>
            <textarea
              value={text} onChange={(e) => setText(e.target.value)}
              placeholder="Ej: El agua está subiendo rápido por la calle..."
              style={{
            ...
            {/* Summary */}
            <div style={{
              marginTop: 20, background: "var(--bg-elevated)", borderRadius: "var(--radius-md)",
              padding: "16px", border: "1px solid var(--border)",
            }}>
              <div style={{ fontSize: "11px", color: "var(--text-faint)", marginBottom: 10, textTransform: "uppercase", letterSpacing: "1.5px", fontWeight: 600 }}>
                Resumen del reporte
              </div>
