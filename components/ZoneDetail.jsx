...
export default function ZoneDetail({ zone, severity, reports, onBack, onReport, onUpvote, pushSupported, isSubscribed, onSubscribe, onUnsubscribe, onLogoClick }) {
  ...
  // Count reports per device for verified badge
  const deviceCounts = {};
  ...
  return (
    <>
      ...
      {expandedPhoto && (
        <div onClick={() => setExpandedPhoto(null)} style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", animation: "fadeIn 0.2s ease" }}>
          <img src={expandedPhoto} alt="Photo" style={{ maxWidth: "95%", maxHeight: "90vh", borderRadius: "12px", objectFit: "contain" }} />
        </div>
      )}
      <div className="screen-slide" style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)", overflow: "hidden" }}>
        ...
        <div style={{ padding: "24px 20px", flex: 1, overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", margin: "16px 0 24px", flexWrap: "wrap" }}>
            ...
            {severity && (
              <button onClick={() => setShareData({ zoneName: zone.name, zoneArea: zone.area, severity, reportText: reports[0]?.text })} style={{
                padding: "6px 14px", borderRadius: "20px",
            ...
            {reports.map((r, i) => {
              ...
              return (
                <div key={r.id} className="card-interactive" style={{
                  ...
                  {r.photo_url && (
                    <div onClick={() => setExpandedPhoto(r.photo_url)} style={{ marginBottom: "10px", borderRadius: "var(--radius-sm)", overflow: "hidden", border: "1px solid var(--border)", cursor: "pointer" }}>
                      <img src={r.photo_url} alt="Evidencia" style={{ width: "100%", maxHeight: 200, objectFit: "cover", display: "block" }} loading="lazy" />
                    </div>
                  )}
        ...
        <div style={{ padding: "14px 20px", background: "rgba(8,13,24,0.95)", backdropFilter: "blur(16px)", borderTop: "1px solid var(--border)", flexShrink: 0 }}>
          <button onClick={onReport} style={{ width: "100%", padding: "15px", background: "linear-gradient(135deg, #D42A2A, #c42222)", color: "#fff", border: "none", borderRadius: "var(--radius-md)", fontSize: "15px", fontWeight: 700, boxShadow: "0 6px 20px rgba(212,42,42,0.25)" }}>
            {t.reportThisZone}
          </button>
        </div>
      </div>
