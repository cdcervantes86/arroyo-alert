...
export default function ZoneDetail({ zone, severity, reports, onBack, onReport, onUpvote, pushSupported, isSubscribed, onSubscribe, onUnsubscribe, onLogoClick, zoneWatchers, prediction, onWatchZone, onUnwatchZone }) {
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, overflowY: "auto", WebkitOverflowScrolling: "touch", background: "var(--bg)" }}>
        ...
        <div style={{ padding: "20px 20px calc(20px + env(safe-area-inset-bottom, 20px))" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: "14px", marginBottom: "4px" }}>
            <div style={{ width: 48, height: 48, borderRadius: "var(--radius-md)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "26px", flexShrink: 0, background: severity ? `${SEVERITY[severity].color}08` : "rgba(255,255,255,0.02)", border: `1px solid ${severity ? SEVERITY[severity].color + "15" : "var(--border)"}` }}>{severity ? SEVERITY[severity].emoji : "⚪"}</div>
            <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: "10px", margin: "12px 0 16px", flexWrap: "wrap" }}>
            {watcherCount > 1 && (
            ...
            {prediction && prediction.score >= 20 && !severity && (
              <div style={{
                ...
                color: prediction.score >= 70 ? "var(--danger)" : prediction.score >= 40 ? "var(--caution)" : "var(--accent)",
              }}>
          ...
          {/* Badges row */}
          <div style={{ display: "flex", gap: "8px", alignItems: "center", margin: "0 0 20px", flexWrap: "wrap" }}>
          ...
          {/* Report button */}
          <button onClick={onReport} style={{ width: "100%", padding: "16px", marginBottom: "28px", background: "linear-gradient(135deg, #D42A2A, #b91c1c)", color: "#fff", border: "none", borderRadius: "var(--radius-md)", fontSize: "16px", fontWeight: 700, boxShadow: "0 6px 20px rgba(212,42,42,0.25)" }}>{t.reportThisZone}</button>
