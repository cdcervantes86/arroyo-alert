...
import { SEVERITY, getZoneDesc, getSevLabel, translateReportText } from "@/lib/zones";
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)", overflow: "hidden" }}>
      <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: "12px", borderBottom: "1px solid var(--border)", flexShrink: 0, background: "rgba(8,13,24,0.92)", backdropFilter: "blur(16px)" }}>
        ...
        <span style={{ flex: 1 }} />
        <button onClick={onBack} style={{ background: "none", border: "none", color: "var(--accent)", fontSize: "14px", fontWeight: 600, padding: "4px 0" }}>{t.backToMap}</button>
      </div>
      ...
      <div style={{ padding: "24px 20px", flex: 1, overflowY: "auto" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: "14px", marginBottom: "4px" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: "4px 0 0", color: "var(--text-dim)", fontSize: "12px" }}>{getZoneDesc(zone, lang)}</p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center", margin: "16px 0 24px", flexWrap: "wrap" }}>
          ...
          {pushSupported && (
            <button onClick={handleToggleSubscribe} style={{ padding: "6px 14px", borderRadius: "20px", background: subscribed ? "rgba(96,165,250,0.1)" : "rgba(255,255,255,0.03)", color: subscribed ? "var(--accent)" : "var(--text-dim)", fontSize: "12px", fontWeight: 600, border: `1px solid ${subscribed ? "rgba(96,165,250,0.2)" : "var(--border)"}`, display: "flex", alignItems: "center", gap: "5px", transition: "all 0.2s ease" }}>
        ...
        {!reports.length && <div style={{ textAlign: "center", padding: "48px 0", color: "var(--text-faint)", fontSize: "13px" }}>{t.noReportsForZone}</div>}

        <div style={{ display: "flex", flexDirection: "column", gap: "10px", paddingBottom: 90 }}>
          {reports.map((r, i) => {
            const isUpvoted = upvoted.has(r.id);
            return (
              <div key={r.id} style={{ background: "var(--bg-card)", border: `1px solid ${SEVERITY[r.severity].color}12`, borderRadius: "var(--radius-md)", padding: "14px 16px", animation: `fadeIn 0.25s ease ${i * 0.05}s both` }}>
      ...
      <div style={{ padding: "14px 20px", background: "rgba(8,13,24,0.95)", backdropFilter: "blur(16px)", borderTop: "1px solid var(--border)", flexShrink: 0 }}>
        <button onClick={onReport} style={{ width: "100%", padding: "15px", background: "linear-gradient(135deg, #D42A2A, #c42222)", color: "#fff", border: "none", borderRadius: "var(--radius-md)", fontSize: "15px", fontWeight: 700, boxShadow: "0 6px 20px rgba(212,42,42,0.25)", letterSpacing: "-0.2px" }}>{t.reportThisZone}</button>
      </div>
    </div>
  );
}
