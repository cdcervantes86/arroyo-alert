...
export default function HeatmapView({ onBack, onLogoClick, onToggleLang }) {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)", overflow: "hidden" }}>
      ...
      <div style={{ padding: "10px 16px", display: "flex", gap: "6px", alignItems: "center", borderBottom: "1px solid rgba(255,255,255,0.06)", flexShrink: 0 }}>
        {timeRanges.map((tr) => (<button key={tr.key} onClick={() => setTimeRange(tr.key)} style={{ padding: "5px 12px", borderRadius: "16px", background: timeRange === tr.key ? "var(--accent-glow)" : "rgba(255,255,255,0.03)", border: `1px solid ${timeRange === tr.key ? "rgba(96,165,250,0.3)" : "rgba(255,255,255,0.06)"}`, color: timeRange === tr.key ? "var(--accent)" : "var(--text-dim)", fontSize: "12px", fontWeight: 600 }}>{tr.label}</button>))}
