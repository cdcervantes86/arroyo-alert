...
function SkeletonCard({ i }) {
  return (
    <div style={{ display: "flex", gap: "12px", alignItems: "center", padding: "14px", marginBottom: "6px", borderRadius: "var(--radius-md)", border: "1px solid var(--border)", animation: `fadeIn 0.3s ease ${i * 0.05}s both` }}>
      <div className="skeleton" style={{ width: 38, height: 38, borderRadius: "var(--radius-sm)", flexShrink: 0 }} />
      <div style={{ flex: 1 }}><div className="skeleton" style={{ width: "55%", height: 13, marginBottom: 8 }} /><div className="skeleton" style={{ width: "35%", height: 10 }} /></div>
    </div>
  );
}
...
function BottomNav({ activeTab, onTab, onReport, liveCount, lang }) {
  const tabs = [
    ...
    { key: "report", icon: null, label: lang === "es" ? "Reportar" : "Report", isReport: true },
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button key={tab.key} onClick={() => onTab(tab.key)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "4px", background: "none", border: "none", padding: "6px 0", position: "relative" }}>
            ...
            <span style={{ fontSize: "9px", fontWeight: isActive ? 700 : 500, color: isActive ? "var(--accent)" : "var(--text-faint)", transition: "color 0.15s", letterSpacing: "0.2px" }}>{tab.label}</span>
        ...
        {items.map((item) => (
          <button key={item.key} onClick={() => { onSelect(item.key); onClose(); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: "14px", padding: "13px 12px", background: "none", border: "none", textAlign: "left", borderRadius: "var(--radius-sm)" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "12px", color: "var(--text-dim)", marginTop: 1 }}>{item.desc}</div>
            </div>
          </button>
        ))}
      </div>
...
function AppContent() {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--bg)", overflow: "hidden" }}>
      {/* HEADER */}
      <div style={{ padding: "10px 16px", display: "flex", alignItems: "center", gap: "8px", background: "rgba(7,11,20,0.95)", backdropFilter: "blur(16px)", borderBottom: "1px solid var(--border)", zIndex: 10, flexShrink: 0 }}>
        ...
        <button onClick={toggleLang} style={{ padding: "4px 8px", borderRadius: "6px", background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", color: "var(--text-dim)", fontSize: "10px", fontWeight: 700, letterSpacing: "0.5px", textTransform: "uppercase", flexShrink: 0 }}>
          {lang === "es" ? "EN" : "ES"}
        </button>
        {isDesktop && (
          <div style={{ display: "flex", background: "rgba(255,255,255,0.03)", borderRadius: "8px", overflow: "hidden", border: "1px solid var(--border)" }}>
            {desktopTabs.map((tab) => {
              ...
              return (
                <button key={tab.key} onClick={() => handleDesktopTab(tab.key)} style={{ padding: "6px 12px", fontSize: "12px", border: "none", background: isActive ? "var(--accent-glow)" : "transparent", color: isActive ? "var(--accent)" : "var(--text-faint)", fontWeight: isActive ? 600 : 400, display: "flex", alignItems: "center", gap: "4px", position: "relative" }}>
                  {tab.icon}
                  {tab.key === "live" && liveCount > 0 && !isActive && <span style={{ position: "absolute", top: 2, right: 2, width: 5, height: 5, borderRadius: "50%", background: "var(--danger)", animation: "blink 1.5s ease-in-out infinite" }} />}
                </button>
              );
            })}
          </div>
      ...
      <div style={{ padding: "8px 16px", display: "flex", gap: "6px", alignItems: "center", flexShrink: 0, borderBottom: "1px solid var(--border)", background: "var(--bg)" }}>
        {dangerCount > 0 && (
          <button onClick={() => handleFilterClick("danger")} style={{ display: "flex", alignItems: "center", gap: "6px", background: activeFilter === "danger" ? "rgba(239,68,68,0.12)" : "var(--danger-bg)", padding: "5px 12px", borderRadius: "20px", border: activeFilter === "danger" ? "2px solid var(--danger)" : "1px solid var(--danger-border)", cursor: "pointer" }}>
        ...
        {cautionCount > 0 && (
          <button onClick={() => handleFilterClick("caution")} style={{ display: "flex", alignItems: "center", gap: "6px", background: activeFilter === "caution" ? "rgba(234,179,8,0.12)" : "var(--caution-bg)", padding: "5px 12px", borderRadius: "20px", border: activeFilter === "caution" ? "2px solid var(--caution)" : "1px solid var(--caution-border)", cursor: "pointer" }}>
            <span style={{ fontSize: "12px", color: "#fde047", fontWeight: 600 }}>{cautionCount} {t.caution}</span>
          </button>
        )}
        ...
        {activeFilter && (
          <button onClick={() => setActiveFilter(null)} style={{ display: "flex", alignItems: "center", background: "rgba(255,255,255,0.03)", padding: "5px 10px", borderRadius: "20px", border: "1px solid var(--border)", cursor: "pointer", fontSize: "11px", color: "var(--text-dim)", fontWeight: 500 }}>✕</button>
        )}
      </div>
        ...
        <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
          {currentMainView === "map" ? (
            <>
              ...
              {!hasAnyReports && (
                <div style={{ position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)", background: "var(--bg-glass)", backdropFilter: "blur(8px)", padding: "10px 20px", borderRadius: "var(--radius-xl)", border: "1px solid var(--border)", fontSize: "13px", color: "var(--text-dim)", fontWeight: 500, pointerEvents: "none", zIndex: 5, animation: "fadeIn 0.5s ease 1s both", whiteSpace: "nowrap" }}>
              ...
              {isDesktop && (
                <div style={{ position: "absolute", bottom: 28, left: "50%", transform: "translateX(-50%)", zIndex: 20 }}>
                  <button onClick={() => setScreen("report")} style={{ padding: "14px 28px", background: "linear-gradient(135deg, #D42A2A, #b91c1c)", color: "#fff", border: "none", borderRadius: "50px", fontSize: "14px", fontWeight: 700, boxShadow: "0 8px 28px rgba(212,42,42,0.3), 0 0 0 1px rgba(255,255,255,0.04)", display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              {loading ? Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} i={i} />) : (
                <>
                  {ZONES.filter((z) => !activeFilter || getZoneSeverity(z.id, reports) === activeFilter).map((z, i) => {
                    ...
                    const accentStyle = sv ? { borderLeft: `3px solid ${c.color}` } : {};
                    return (
                      <button key={z.id} onClick={() => handleZoneClick(z.id)} className="card-interactive" style={{
                        width: "100%", background: "var(--bg-card)",
                        border: "1px solid var(--border)", ...accentStyle,
                        borderRadius: "var(--radius-md)", padding: "13px 14px",
                        textAlign: "left", display: "flex", gap: "12px", alignItems: "center",
                        marginBottom: "6px", animation: `fadeIn 0.25s ease ${i * 0.03}s both`,
                      }}>
                        <div style={{ width: 38, height: 38, borderRadius: "var(--radius-sm)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", flexShrink: 0, background: c ? `${c.color}08` : "rgba(255,255,255,0.02)", border: `1px solid ${c ? c.color + "15" : "var(--border)"}` }}>
                          {c ? c.emoji : "⚪"}
                        </div>
                        ...
                        {zr.length > 0 && <span style={{ fontSize: "11px", color: c ? c.color : "var(--text-dim)", background: c ? `${c.color}0a` : "rgba(255,255,255,0.03)", padding: "3px 8px", borderRadius: "6px", flexShrink: 0, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{zr.length}</span>}
                        <span style={{ color: "var(--text-faint)", fontSize: "14px", flexShrink: 0 }}>›</span>
                      </button>
                    );
                  })}
                  <div style={{ textAlign: "center", padding: "36px 0 16px", fontSize: "12px", color: "var(--text-faint)" }}>{lang === "es" ? "Hecho para Barranquilla 🇨🇴" : "Made for Barranquilla 🇨🇴"}</div>
                </>
              )}
            </div>
          ) : (
