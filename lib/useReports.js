...
export function useReports(onEvent) {
  ...
  const upvoteReport = useCallback(async (reportId) => {
    ...
    // No optimistic update here — the UI renders r.upvotes + 1 when the
    // user has upvoted locally, and the realtime UPDATE event brings in
    // the new count a moment later. Doing setReports here would double-count.
    await supabase.rpc("increment_report_upvote", { report_id: reportId });
  }, []);
